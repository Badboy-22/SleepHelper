...
function toISO(dtLocal){ if(!dtLocal) return null; try{ return new Date(dtLocal).toISOString(); }catch{ return null; } }
...
async function loadFor(dateStr){
  ...
  const ul=$('sch-list'); ul.innerHTML = items.length? '' : '<li>일정 없음</li>';
  items.forEach(it=>{ const li=document.createElement('li'); li.innerHTML=`<span class="time">${fmt(it.start)} ~ ${fmt(it.end)}</span> · ${it.title||''}`; ul.appendChild(li); });
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  $('sch-save').addEventListener('click', async ()=>{
    const title=$('sch-title').value.trim(); const start=toISO($('sch-start').value); const end=toISO($('sch-end').value); const note=$('sch-note').value||null;
    ...
    const r=await fetch('/api/schedule',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({ title, start, end, note })});
    const data=await r.json().catch(()=>({})); $('sch-msg').textContent=r.ok?'Saved.':(data.error||'error');
    if(r.ok){ $('sch-title').value=''; $('sch-start').value=''; $('sch-end').value=''; $('sch-note').value=''; await loadFor($('view-date').value); }
  });
